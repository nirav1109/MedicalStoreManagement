...
$currentDateTime = date('Y-m-d');
  ...
  $stockArray = $con->query("select stock ,unit from inventeries");
   while ($row = $stockArray->fetch_assoc()) 
            {
              $oldstock = $row['stock'];
              $oldunit = $row['unit'];
            }
  $s =$oldstock + $_POST['stock'];
  $u = $oldunit + $_POST['stock'];
  $sql = "update inventeries set stock =".$s.", unit = ".$u." where name ='$n'";
    ...
    <div style="width: 55%;margin: auto;padding: 22px;" class="well well-sm center">
      ...
      <form method="POST">
         <div class="form-group">
            <label for="some" class="col-form-label">Select Medicine</label>
            <select class="form-control" required name="name">
              <option selected disabled value="">Please Select Medicine</option>
            <?php getAllMedicine(); ?>
              
            </select>
          
          <div class="form-group">
            <label for="some" class="col-form-label">stock</label>
            <input type="text" name="stock"  class="form-control" id="some" required>
          </div>
